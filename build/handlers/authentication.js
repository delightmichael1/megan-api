...
const handleDeviceAuth = async ({ req, accessToken, refreshToken, userId }) => {
    try {
        ...
        if (!deviceData) {
            ...
            // Update device
            const modified = await device.updateOne({ deviceID: deviceId }, {
                $set: {
                    online: true,
                    deviceToken: accessToken,
                    refreshToken: refreshToken,
...
setInterval(() => {
    ...
    for (const [deviceId, entry] of refreshTokenCache.entries()) {
        if (now > entry.expiration)
            refreshTokenCache.delete(deviceId);
    }
    for (const [deviceId, entry] of accessTokenCache.entries()) {
        if (now > entry.expiration)
            accessTokenCache.delete(deviceId);
    }
}, 60_000);
