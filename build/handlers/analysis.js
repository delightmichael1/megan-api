...
export const getDashboardDataByUserId = async (req, res) => {
    try {
...
// Helper functions
function calculateHoursCompleted(activities, lessons, modules) {
...
function calculateCompletionRate(results, modules) {
    if (modules.length === 0)
        return 0;
...
function calculateUserProgress(activities, lessons, modules) {
...
function getUpcomingSessions(lessons, activities) {
    const now = new Date();
    return lessons
        .filter(lesson => {
...
function getFeaturedTraining(lessons) {
    return lessons
...
function formatRecentActivity(activities, lessons) {
    return activities
        .slice(0, 5)
        .map(activity => {
...
function getBookmarkedLessons(activities, lessons) {
    const enrolledLessons = activities
        .filter(a => a.type === 'enrollment')
        .map(a => a.course);
    const completedLessons = activities
