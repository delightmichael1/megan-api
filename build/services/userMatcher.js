...
class UserMatcher {
    ...
    async findMatches(userId, options = {}) {
        const { limit = 20, skip = 0, minScore = 0.3, mentorshipType = null, industry = null, location = null, onlineOnly = false } = options;
        try {
     ...
     */
    calculateMatchScore(currentUser, targetUser) {
        const scores = {
     ...
     */
    calculateMentorshipCompatibility(currentUser, targetUser) {
        const currentType = currentUser.mentorshipType;
        const targetType = targetUser.mentorshipType;
        // Perfect matches
        if (currentType === 'mentor' && targetType === 'mentee')
            return 1.0;
        if (currentType === 'mentee' && targetType === 'mentor')
            return 1.0;
        // Both flexible (both type)
        if (currentType === 'both' && targetType === 'both')
            return 0.9;
     ...
     * @returns {Number} Score between 0 and 1
     */
    calculateSkillsMatch(currentUser, targetUser) {
        ...
        const intersection = currentSkillsLower.filter((skill) => targetSkillsLower.includes(skill));
        ...
        const complementarySkills = targetSkillsLower.filter((skill) => !currentSkillsLower.includes(skill));
     ...
     * @returns {Number} Score between 0 and 1
     */
    calculateIndustryMatch(currentUser, targetUser) {
     ...
     * @returns {Number} Score between 0 and 1
     */
    calculateLocationProximity(currentUser, targetUser) {
     ...
     * @returns {Number} Score between 0 and 1
     */
    calculateRatingScore(targetUser) {
     ...
     * @returns {Number} Score between 0 and 1
     */
    calculateActivityScore(targetUser) {
     ...
     * @returns {Number} Score between 0 and 1
     */
    calculateOnlineStatus(targetUser) {
     ...
     */
    getMatchFactors(currentUser, targetUser) {
        return {
            mentorshipCompatibility: this.calculateMentorshipCompatibility(currentUser, targetUser),
            skillsMatch: this.calculateSkillsMatch(currentUser, targetUser),
            industryMatch: this.calculateIndustryMatch(currentUser, targetUser),
...
export async function findUserMatches(userId, options = {}) {
    const matcher = new UserMatcher();
    return await matcher.findMatches(userId, options);
}
export async function findMentors(userId, options = {}) {
    const matcher = new UserMatcher();
...
export async function findMentees(userId, options = {}) {
    const matcher = new UserMatcher();
    return await matcher.findMatches(userId, {
        ...options,
...
export async function findIndustryPeers(userId, industry, options = {}) {
    const matcher = new UserMatcher();
    return await matcher.findMatches(userId, {
        ...options,
        industry,
...
//# sourceMappingURL=userMatcher.js.map